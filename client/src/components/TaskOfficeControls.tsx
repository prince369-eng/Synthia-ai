...
export function TaskQualityBudgetPanel({ taskId, qualityBudgets, readOnly }: { taskId: string; qualityBudgets: QualityBudget[]; readOnly: boolean }) {
  ...
  const error = create.error?.message ?? update.error?.message ?? archive.error?.message;
  ...
  return <section className="space-y-3" aria-label="Task quality budgets">
    ...
    {error ? <p role="alert" className="rounded-lg border border-rose-300/20 bg-rose-300/[.05] p-3 text-[11px] text-rose-200">{error}</p> : null}
