...
export function SettingsGeneral({ preferences, theme, onToggleTheme, onSave, saving, persistenceAvailable }: { preferences: Preferences; theme: "light" | "dark"; onToggleTheme?: () => void; onSave: (patch: Preferences) => void; saving: boolean; persistenceAvailable: boolean }) {
  ...
  return <div><SectionHeading icon={Settings2} title="General" description="Choose the default conditions Synthia applies when you begin a new autonomous task." /><div className="mt-6 grid gap-3 lg:grid-cols-2"><SettingsCard title="Appearance" detail="Your chosen display mode is stored locally on this device."><PreferenceSwitch label="Dark appearance" description={theme === "dark" ? "Radiant orange dark workspace is active." : "Light workspace is active."} enabled={theme === "dark"} onChange={() => { onToggleTheme?.(); if (persistenceAvailable) onSave({ appearanceTheme: theme === "dark" ? "light" : "dark" }); }} disabled={saving || !onToggleTheme} /></SettingsCard><SettingsCard title="Default review mode" detail="Applied to each new task, before its first action."><div className="flex flex-wrap gap-2"><Button size="sm" className={cn(defaults.mode === "ask_before_risky" ? "synthia-primary-button" : "bg-white/5 text-[#d9c9ba]")} onClick={() => saveDefaults({ mode: "ask_before_risky" })} disabled={saving || !persistenceAvailable}>Ask before risky</Button><Button size="sm" className={cn(defaults.mode === "supervised" ? "synthia-primary-button" : "bg-white/5 text-[#d9c9ba]")} onClick={() => saveDefaults({ mode: "supervised" })} disabled={saving || !persistenceAvailable}>Supervised</Button></div></SettingsCard></div><div className="mt-4"><SettingsCard title="Default task capabilities" detail="These defaults are sent with new task requests. Each task remains subject to server-side approval and provider availability."><div className="grid gap-2 sm:grid-cols-3"><PreferenceSwitch label="Web research" description="Allow configured search tools." enabled={defaults.allowWebSearch} onChange={enabled => saveDefaults({ allowWebSearch: enabled })} disabled={saving || !persistenceAvailable} /><PreferenceSwitch label="Code execution" description="Permit sandboxed code work." enabled={defaults.allowCodeExecution} onChange={enabled => saveDefaults({ allowCodeExecution: enabled })} disabled={saving || !persistenceAvailable} /><PreferenceSwitch label="File writes" description="Permit sandbox artifact writes." enabled={defaults.allowFileWrites} onChange={enabled => saveDefaults({ allowFileWrites: enabled })} disabled={saving || !persistenceAvailable} /></div></SettingsCard></div></div>;
}
