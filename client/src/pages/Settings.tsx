...
export default function Settings() {
  ...
  const workosReadiness = trpc.auth.workosReadiness.useQuery(undefined, { enabled: section === "account", retry: false });
  ...
  const skillsMutationError = [createSkill.error, updateSkill.error, setSkillEnabled.error, deleteSkill.error, createSkillDraft.error, createSkillDraftFromExample.error, createSkillDraftFromTask.error].find(Boolean);
  const personalizationMutationError = [updatePersonalization.error, addPersonalizationMemory.error, updatePersonalizationMemory.error, deletePersonalizationMemory.error, clearSessionPersonalization.error].find(Boolean);
    ...
    <main className="synthia-utility-page">
      ...
      <div className="mt-5 grid items-start gap-4 xl:grid-cols-[184px_minmax(0,960px)] xl:justify-start">
        ...
        <section className="synthia-settings-panel" aria-live="polite">
          ...
          {updatePreferences.isError ? <p role="alert" className="mb-4 text-xs text-rose-300">{clientErrorMessage(updatePreferences.error, "Workspace preferences could not be saved. Please try again.")}</p> : null}
          ...
          {section === "account" ? <SettingsAccount user={user} hasCompletedOnboarding={Boolean(settings.data?.hasCompletedOnboarding)} onManageAccount={() => startLogin()} workosReadiness={workosReadiness.data} workosLoading={workosReadiness.isLoading} workosError={workosReadiness.isError} /> : null}
          ...
          {section === "services" ? <SettingsServices integrations={integrations.data ?? []} loading={integrations.isLoading} error={integrations.isError} /> : null}
          {section === "integrations" ? <SettingsIntegrations integrations={integrations.data ?? []} loading={integrations.isLoading} error={integrations.isError} removeIntegration={removeIntegration} /> : null}
...
export function SettingsGeneral({ preferences, theme, onToggleTheme, onSave, saving, persistenceAvailable }: { preferences: Preferences; theme: "light" | "dark"; onToggleTheme?: () => void; onSave: (patch: Preferences) => void; saving: boolean; persistenceAvailable: boolean }) {
  ...
  return <div><SectionHeading icon={Settings2} title="General" description="Choose the default conditions Synthia applies when you begin a new autonomous task." /><div className="mt-6 grid gap-3 lg:grid-cols-2"><SettingsCard title="Appearance" detail="Your chosen display mode is stored locally on this device."><PreferenceSwitch label="Dark appearance" description={theme === "dark" ? "Teal and cyan dark workspace is active." : "Light workspace is active."} enabled={theme === "dark"} onChange={() => { onToggleTheme?.(); if (persistenceAvailable) onSave({ appearanceTheme: theme === "dark" ? "light" : "dark" }); }} disabled={saving || !onToggleTheme} /><div className="mt-4 border-t border-white/8 pt-3"><p className="text-xs font-semibold text-[#dff4ef]">Workspace density</p><p className="mt-1 text-[.66rem] leading-5 text-[#7f9790]">Controls spacing only; it never changes permissions or review requirements.</p><div className="mt-3 synthia-density-switch" role="group" aria-label="Workspace density"><button type="button" className={cn(workspaceDensity === "comfortable" && "is-active")} aria-pressed={workspaceDensity === "comfortable"} onClick={() => onSave({ workspaceDensity: "comfortable" })} disabled={saving || !persistenceAvailable}>Comfortable<span>More room to scan</span></button><button type="button" className={cn(workspaceDensity === "compact" && "is-active")} aria-pressed={workspaceDensity === "compact"} onClick={() => onSave({ workspaceDensity: "compact" })} disabled={saving || !persistenceAvailable}>Compact<span>More content in view</span></button></div></div></SettingsCard><SettingsCard title="Default review mode" detail="Applied to each new task, before its first action."><div className="synthia-review-mode grid grid-cols-2 gap-2"><Button size="sm" className={cn("mt-0", defaults.mode === "ask_before_risky" ? "synthia-primary-button" : "bg-white/5 text-[#c7ded8]")} onClick={() => saveDefaults({ mode: "ask_before_risky" })} disabled={saving || !persistenceAvailable}>Ask before risky</Button><Button size="sm" className={cn("mt-0", defaults.mode === "supervised" ? "synthia-primary-button" : "bg-white/5 text-[#c7ded8]")} onClick={() => saveDefaults({ mode: "supervised" })} disabled={saving || !persistenceAvailable}>Supervised</Button></div></SettingsCard></div><div className="mt-4"><SettingsCard title="Default task capabilities" detail="These defaults are sent with new task requests. Each task remains subject to server-side approval and provider availability."><div className="grid min-w-0 gap-3 md:grid-cols-3">{[
...
export function SettingsAccount({ user, hasCompletedOnboarding, onManageAccount, workosReadiness, workosLoading = false, workosError = false }: { user: { id: number; name?: string | null; email?: string | null; role?: string } | null | undefined; hasCompletedOnboarding: boolean; onManageAccount?: () => void; workosReadiness?: { configured: boolean; active: boolean; missing: string[]; status: string }; workosLoading?: boolean; workosError?: boolean }) {
  const optionalSignInStatus = workosReadiness?.active ? "Enabled" : workosReadiness?.configured ? "Configured and safely disabled" : "Setup not checked";
  return <div><SectionHeading icon={UserRound} title="Account" description="Identity and session data are managed through Synthia’s secure account boundary." /><div className="mt-6 grid gap-3 sm:grid-cols-2"><SettingSummary label="Signed-in identity" value={user?.name || user?.email || "Authenticated Synthia user"} /><SettingSummary label="Account email" value={user?.email || "Not provided by the identity provider"} /><SettingSummary label="Workspace role" value={user?.role === "admin" ? "Administrator" : "Workspace member"} /><SettingSummary label="Onboarding" value={hasCompletedOnboarding ? "Completed" : "Not yet completed"} /></div><div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/8 bg-white/[.025] px-4 py-3"><p className="max-w-xl text-xs leading-5 text-[#a99a8a]">Account changes and sign-in methods remain within the verified authentication portal. Synthia does not store identity-provider passwords or tokens.</p><Button size="sm" variant="outline" onClick={onManageAccount}>Manage account</Button></div><SettingsCard title="Optional enterprise sign-in" detail="A separate provider can be prepared without replacing the current authenticated session."><div className="synthia-auth-readiness">{workosLoading ? <LoadingRow label="Checking optional sign-in readiness…" /> : workosError ? <p role="alert" className="text-xs leading-5 text-rose-300">Optional sign-in readiness is unavailable. Your current session is unaffected.</p> : <><div><p className="font-medium text-[#e5f2ef]">{optionalSignInStatus}</p><p>{workosReadiness?.configured ? "The optional provider remains disabled until an explicit migration and activation decision." : `Still needed: ${workosReadiness?.missing.join(", ") || "configuration"}.`}</p></div><span className={cn("synthia-readiness-badge", workosReadiness?.configured && "is-ready")}>{workosReadiness?.active ? "Enabled" : workosReadiness?.configured ? "Prepared" : "Not active"}</span></>}</div><p className="mt-3 text-xs leading-5 text-[#839792]">Current sign-in remains the only active path. No user, session, or identity data is created here.</p></SettingsCard></div>;
}
...
function SettingsDataControls() { return <div><SectionHeading icon={ShieldCheck} title="Data controls" description="Synthia protects task records and user-owned attachments with secure workspace data handling." /><div className="mt-6 grid gap-3 sm:grid-cols-2"><SettingsCard title="Task data" detail="Task state, event replay, approvals, and usage records remain available for your workspace history."><p className="text-xs leading-5 text-[#a99a8a]">Deletion and retention controls are not available in this workspace yet.</p></SettingsCard><SettingsCard title="Files and artifacts" detail="Attachment and deliverable access is checked against the authenticated owner before a signed URL is issued."><p className="text-xs leading-5 text-[#a99a8a]">Sensitive access details and file locations are never shown in Settings.</p></SettingsCard></div></div>; }
function SettingsDeployments() { return <div><SectionHeading icon={ServerCog} title="Deployments" description="Managed releases stay separate from task work while preserving reviewable source and checkpoint history." /><div className="mt-6"><SettingsCard title="Managed deployment" detail="Publish and release actions stay in the project management area so changes remain reviewable."><p className="text-xs leading-5 text-[#a99a8a]">Project publishing and domain controls are managed separately from the task workspace.</p></SettingsCard></div></div>; }
function SettingsDeveloper() { return <div><SectionHeading icon={KeyRound} title="Developer" description="Developer access is protected and must be explicitly authorized before it can use workspace data or actions." /><div className="mt-6 grid gap-3 sm:grid-cols-2"><SettingsCard title="Secure developer access" detail="Connection details are managed securely outside this workspace."><p className="text-xs leading-5 text-[#a99a8a]">Synthia shows capability availability only; sensitive values never appear in this workspace.</p></SettingsCard><SettingsCard title="API and webhooks" detail="Public API and webhook controls are not available in this workspace yet."><p className="text-xs leading-5 text-[#a99a8a]">This keeps third-party access scoped until approved developer access options are available.</p></SettingsCard></div></div>; }
function SettingsMemory({ memory, loading, error }: { memory: Array<any>; loading: boolean; error: boolean }) { return <div><SectionHeading icon={Sparkles} title="Personalization" description="Review durable task-relevant facts that Synthia may use to improve future work." />{loading ? <LoadingRow label="Loading durable memory…" /> : null}{error ? <SettingsUnavailable message="Personalization is temporarily unavailable. Refresh the workspace or try again shortly." /> : null}{!loading && !error ? <><p className="mt-5 text-sm text-[#a99a8a]">There are {memory.length} active memory records.</p><div className="mt-4 space-y-2">{memory.map(item => <div key={item.id} className="rounded-xl border border-white/8 bg-white/[.025] p-3"><p className="text-sm text-[#eee2d3]">{item.fact}</p><p className="mt-1 text-xs text-[#8d7d6e]">{item.scope}</p></div>)}</div>{memory.length === 0 ? <div className="mt-4 rounded-xl border border-dashed border-white/12 p-5 text-sm text-[#968677]">No durable facts have been created.</div> : null}</> : null}</div>; }
function SettingsSecurity() { return <div><SectionHeading icon={ShieldCheck} title="Security" description="Controls that protect tasks, artifacts, connections, and approvals." /><div className="mt-5 space-y-3 text-sm leading-6 text-[#b2a293]"><p className="flex gap-2"><CheckCircle2 size={17} className="mt-0.5 text-emerald-300" />Task ownership is enforced for task, event, approval, and artifact requests.</p><p className="flex gap-2"><CheckCircle2 size={17} className="mt-0.5 text-emerald-300" />High-risk external actions require a durable approval record before execution.</p><p className="flex gap-2"><CheckCircle2 size={17} className="mt-0.5 text-emerald-300" />Connected-service tokens use envelope encryption when saved through an enabled connection flow.</p><p className="flex gap-2"><CheckCircle2 size={17} className="mt-0.5 text-emerald-300" />Task and artifact data remains private to the authenticated workspace owner unless Synthia adds an ownership-checked sharing contract.</p></div></div>; }
