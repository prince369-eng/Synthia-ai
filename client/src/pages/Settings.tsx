...
export function SettingsSectionNav({ section, onNavigate }: { section: string; onNavigate: (path: string) => void }) {
  ...
  return <nav className="synthia-settings-nav" aria-label="Settings sections"><label className="relative mb-3 block"><Search size={14} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[#867668]" /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search settings" aria-label="Search settings" className="h-8 w-full rounded-md border border-white/10 bg-black/15 pl-8 pr-2 text-xs text-[#eadbca] outline-none placeholder:text-[#75665a] focus:border-orange-300/60" /></label>{visibleGroups.map(group => <div key={group.label} className="synthia-settings-group"><p>{group.label}</p>{group.sections.map(item => { const Icon = item.icon; return <button key={item.id} onClick={() => onNavigate(settingsPath(item.id))} className={cn(section === item.id && "active")}><Icon size={14} />{item.label}</button>; })}</div>)}{visibleGroups.length === 0 ? <p className="px-2 text-xs leading-5 text-[#867668]">No matching settings.</p> : null}</nav>;
}
