    ...
    <div className="grid min-h-[calc(100vh-3.5rem)] lg:grid-cols-[minmax(320px,.9fr)_minmax(480px,1.2fr)]">
      <section className="flex min-h-0 flex-col border-r border-white/8">
        ...
        <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-4 py-4 sm:px-5">
          ...
          {data.messages.map(entry => <article key={entry.id} className={cn("max-w-[92%] rounded-xl px-3 py-2.5", entry.role === "user" ? "ml-auto bg-orange-400/12 text-orange-50" : "border border-white/8 bg-white/[.035] text-[#e6dbce]")}><p className="mb-1 text-[9px] font-semibold uppercase tracking-[.13em] text-[#9d8d7d]">{entry.role === "agent" ? "Synthia" : entry.role}</p><p className="whitespace-pre-wrap text-xs leading-5">{entry.content}</p><time className="mt-1.5 block text-[9px] text-[#7c6e60]">{localTime(entry.createdAt)}</time></article>)}
        ...
        {!replayMode ? <form onSubmit={sendMessage} className="border-t border-white/8 p-3"><label className="sr-only" htmlFor="task-message">Send a message to Synthia</label><div className="flex gap-2"><Input id="task-message" value={message} onChange={event => setMessage(event.target.value)} placeholder="Add a constraint or answer Synthia…" className="h-8 border-white/10 bg-[#1c1510] text-xs text-[#f5e9da] placeholder:text-[#77695c]" /><Button type="submit" size="icon" disabled={!message.trim() || addMessage.isPending} className="h-8 w-8 shrink-0 bg-orange-400 text-[#2a170b] hover:bg-orange-300"><Send size={14} /></Button></div>{addMessage.isError ? <p role="alert" className="mt-2 text-[11px] text-rose-300">{addMessage.error.message}</p> : null}</form> : null}
      </section>
      <aside className="flex min-h-0 flex-col bg-[#19130f]" aria-label="Agent's Computer">
        <div className="flex items-center justify-between border-b border-white/8 px-4 py-3"><div><p className="flex items-center gap-2 text-xs font-semibold text-[#f6eadb]"><Bot size={15} className="text-orange-300" />Agent’s Computer</p><p className="mt-0.5 text-[10px] text-[#887869]">Open workspace · <span className="text-orange-200">Code</span> selected</p></div>{data.sandboxes[0] ? <span className="rounded-full bg-emerald-400/10 px-2 py-1 text-[9px] text-emerald-300">Sandbox {data.sandboxes[0].status}</span> : <span className="rounded-full bg-white/5 px-2 py-1 text-[9px] text-[#9c8c7d]">No sandbox yet</span>}</div>
        <div className="flex overflow-x-auto border-b border-white/8 px-2" role="tablist" aria-label="Agent workspace panels">{tabs.map(item => { const Icon = item.icon; return <button key={item.id} role="tab" aria-selected={tab === item.id} onClick={() => setTab(item.id)} className={cn("flex h-9 shrink-0 items-center gap-1.5 border-b-2 px-2.5 text-[11px]", tab === item.id ? "border-orange-400 text-orange-200" : "border-transparent text-[#887869] hover:text-[#d6c7b5]")}><Icon size={13} />{item.label}</button>; })}</div>
