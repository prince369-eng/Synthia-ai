...
|---|---|---|---|---|
| P0 | Production dependency advisories | **Needs remediation** | Production audit reported one moderate and three high-severity transitive advisories. | Refresh the lockfile against the existing safe `form-data` override; then address the ExcelJS and PPTX dependency chains or constrain untrusted-file processing until an upstream-safe replacement is selected. |
...
| P2 | Historical tracker and documentation | **Needs consolidation** | Active implementation is documented and tested, but the historical tracker contains many stale unchecked entries. | Reconcile old entries into a smaller maintained roadmap; update status documents that still say the Network Lab schema migration is pending. |
...
The production dependency audit found four transitive advisories. The workspace already declares a `form-data` override to the fixed range, but the lockfile still resolves version 4.0.4 through the Hyperbrowser SDK tree. This is the most direct P0 repair because the intended constraint is already explicit and the outcome can be checked deterministically.

The remaining advisories are inherited through `exceljs` (`uuid`) and `pptxgenjs` (`image-size`). The latter currently has no patched release identified by the audit output. It should not be “fixed” by a blind override: the safe next step is to inspect whether user-controlled Office or image input reaches the affected parser, isolate conversion to a bounded worker where necessary, and choose a maintained dependency upgrade or feature boundary deliberately.

## Recommended sequence

First, reconcile the `form-data` lockfile override and run the full test/build suite. Second, trace the upload, spreadsheet, and presentation paths to determine whether untrusted file bytes reach the vulnerable transitive parsers; introduce strict type/size checks and an isolated execution boundary where that is possible, then replace or upgrade dependencies based on compatibility testing. Third, conduct a real deployment readiness pass for OAuth redirects, worker supervision, Redis reliability, provider quotas, storage, and the integrations the operator actually intends to enable.
