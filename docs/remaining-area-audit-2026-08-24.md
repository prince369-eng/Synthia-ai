...
The production build now assigns React, typed data clients, Radix primitives, icons, and LiveKit to stable vendor chunks. The primary application entry measures approximately 178 KB and the largest route entry approximately 267 KB after minification; the only chunk still over the default 500 KB warning threshold is the approximately 532 KB LiveKit runtime. That runtime is reachable only from the already lazy `VoiceModeDialog` inside the lazy task-workspace route, so it is not part of the first-load request path. It remains intentionally isolated rather than being split through unreviewed vendor internals; further reduction requires an upstream-safe LiveKit strategy and should be assessed separately.

## Development diagnostic privacy boundary

The local development collector can observe browser console events, network outcomes, and semantic UI interactions. Before any such payload is written to the local `.manus-logs/` directory, a server-side minimization boundary now removes console arguments and stack text; request and response URLs, headers, and bodies; exception details; typed values; and element labels or text. The persisted diagnostics retain only bounded metadata needed for local troubleshooting: event timing, safe severity or kind, HTTP method, endpoint class, status, duration, and structural element type. The local endpoint rejects malformed input with a fixed code and rejects a declared or streamed payload above 256 KiB before JSON parsing; it does not echo an exception. The collector remains available to the local development transform but is neither injected nor present in the final production artifact directory.

## Sandbox failure boundary

Sandbox adapter failures now retain only stable operation-level messages for opening a URL, reading a file, writing a file, or creating a checkpoint. Docker command `stderr` and E2B checkpoint provider errors are not propagated into task-facing error paths. The local Docker runner remains configuration-gated and does not start any sandbox during this review; deterministic tests cover the stable failure messages alongside the existing filesystem and command policy tests.

## HTTP response-security boundary

The server already limits credentialed CORS to configured public origins or exact same-origin managed previews, with no wildcard origin path. The response policy now centralizes content security, framing, MIME-sniffing, referrer, and permissions headers in a deterministic contract and applies that contract before any rejected-origin response or preflight completion. It adds `Strict-Transport-Security: max-age=31536000; includeSubDomains` only for production deployment. Development and preview modes deliberately omit HSTS so local HTTP access is not pinned to HTTPS. The JSON and URL-encoded parsers retain their bounded 24 MB ceiling for the supported base64 voice path; malformed bodies now return only `INVALID_REQUEST_BODY` and oversized bodies only `PAYLOAD_TOO_LARGE`, rather than a framework-generated response. Focused header and parser tests, strict TypeScript, the complete deterministic suite, and the production build passed; the remaining large client-chunk build advisories stay visible.
...
## OAuth callback and session boundary

The login flow generates a fresh nonce only at explicit navigation time, stores it in a short-lived host-only secure state cookie, and binds the same nonce to the callback state. The callback rejects missing or mismatched nonce values before token exchange and clears the one-time state cookie after a valid match. The public account-portal base URL must be public HTTPS with no credentials, port, query, or fragment before the client constructs the authorization URL. Session cookies remain HTTP-only, path-scoped, secure only when the request is HTTPS or trusted as forwarded HTTPS, and `SameSite=Lax` to allow the OAuth top-level callback while excluding cross-site subrequests. Deterministic regressions verify portal validation, state construction, nonce comparison, and the state-cookie source contract; no authentication flow or browser action was initiated.

### Reverse-proxy transport gate

The current deployment path may use `X-Forwarded-Proto` for HTTPS-terminated requests, while an arbitrary self-hosted topology must not trust that client-controlled header unless its immediate upstream proxy is explicitly trusted. The correct Express `trust proxy` policy depends on the operator’s exact proxy chain, including whether all public traffic is forced through a single managed gateway. This review therefore makes **no default proxy-hop or trust-policy source change**: setting one without the deployed topology could either permit direct-header spoofing or break secure session delivery behind a valid proxy. Before an external deployment is enabled, the operator must choose and test the explicit reverse-proxy boundary, then validate OAuth callback cookies and task-owned artifact origin construction through that approved path.

## Task-owned artifact storage proxy boundary

The artifact route accepts only normalized relative storage keys: empty segments, absolute paths, backslashes, traversal segments, and oversized values are rejected before authorization. User-scoped input prefixes are bound to the authenticated numeric user identifier, while task attachments and deliverables require a matching durable ownership record before the proxy requests a signed URL. Missing authorization deliberately returns the same unavailable response as an invalid object, avoiding a storage-key existence oracle. The proxy does not pass object bytes through the application: after authorization it redirects only to a public HTTPS signed URL without credentials or a non-default port, and response headers set `no-store`, `no-referrer`, and MIME-sniffing protection. The local regression suite covers key and redirect normalization; no storage request, upload, credential change, or artifact retrieval was performed. Live validation remains deferred until the operator selects and configures storage.

## Public-web destination boundary

Before a public-web URL reaches a sandbox or an enabled public-media understanding route, the server requires HTTP(S), no embedded credentials or explicit port, a non-local hostname, and DNS answers that are exclusively public. The policy rejects literal addresses, localhost and internal naming patterns, cloud-metadata names, private/link-local IPv4, loopback and private IPv6, IPv4-mapped private IPv6 forms, and special-use DNS answers exercised by the regression suite. This keeps validation local to the control plane and does not start a browser, sandbox, or provider request. The remaining operational control is egress enforcement: a deployment that enables public-web work must route outbound DNS and HTTP through an operator-controlled boundary that preserves the same policy after resolution.

## Server error-disclosure boundary

The review found raw transport and provider-detail construction in two configuration-gated capability helpers. Heartbeat transport failures and non-success provider statuses now map to bounded, status-appropriate client messages without endpoint text, response bodies, or raw status detail. Voice-transcription results now return only a stable error message and error code; request, provider, and unexpected exception details are no longer included in the result payload. A terminal Express error boundary now also logs only a stable error classification for unexpected route failures and returns the fixed `INTERNAL_SERVER_ERROR` response instead of default framework exception rendering. Parser-specific malformed and oversized body responses continue to take precedence. The core Data API adapter similarly retains no upstream response body or provider-supplied status text when a configured service fails: it returns only a bounded HTTP status, and absent server-side configuration fails before a request is attempted. Its deterministic regressions use mocked responses only; no configured service request was made. Deterministic regressions verify these payload contracts. The helpers remain configuration-gated and no Heartbeat or transcription request was started during this work.
...
The rate-limit Redis client now follows the same observability boundary. Its connection-error listener records only a stable error class with the event identifier rather than serializing the raw Redis error object, whose message or stack could include transport detail. A source-contract regression prevents reintroducing the raw-error logger field. This local change does not connect to Redis or alter the production fail-closed rate-limit policy.
