...
## 2026-08-19 — External-provider and authentication-entry verification

The provider settings surface now distinguishes **Connected**, **Ready to connect**, and **Credentials required** states. “Connected” is reserved for an authenticated user integration record; configured server-side credentials alone do not claim a user connection. The full environment contract is documented in `docs/environment-reference.md`; actual values remain in secure project secrets or an ignored local environment file rather than version control.

The live Manus account page was inspected at `https://manus.im/login`. It explicitly presents one unified **Sign in or sign up** experience and a **Continue with Google** identity option. Accordingly, Synthia’s Sign in, Create an account, and Continue with Google controls all use the verified `app-auth` SSO protocol with `type=signIn`, preserving callback state while delegating account creation and provider-token handling to the trusted portal. URL-contract tests verify the generated callback, nonce state, application identifier, and supported portal type for each entry intent. A live authenticated Synthia browser review again confirmed the compact dashboard and anchored signed-in profile control after this update.
...
When the user reported a remaining overflow after that capture, the static preview delivery was updated to serve its classic bundle with `Cache-Control: no-store` and a bundle mtime revision query. The connected browser was then reloaded at `/settings/general` after a full rebuild. The fresh bundle rendered all capability switches inside their respective bounded cards and both review-mode controls inside their shared card; no live overflow was visible.

The final static-preview screenshot was captured after the cache-policy rebuild at a 1920 × 1020 viewport. It retains the contained General controls alongside the verified cache-busting contract, which is covered by the static-preview test suite.
