...
| Realtime Voice Mode | The consent-first Voice Mode interface, browser-native local screen sharing, task-scoped session credentials, and task-history event lifecycle are implemented. | Supply LiveKit credentials, confirm Gemini Live access, configure `SYNTHIA_REALTIME_VOICE_ENABLED=true`, and deploy the separate `voice-worker` on always-on/Reserved hosting before a user starts a session. | Browser microphone prompts, room connection, and live model use must remain explicit, user-started, and operator-configured; no real-time media session or provider consumption is permitted during development. |
