...
| Realtime Voice Mode | The consent-first Voice Mode interface, browser-native local screen sharing, task-scoped session credentials, and task-history event lifecycle are implemented. The task-workspace **Voice** entry opens the dialog; **Start voice** requests microphone consent; **Share screen** appears only after a live voice session begins and invokes the browser-native chooser; **Stop sharing** and **End voice** always stop local tracks and leave the realtime room. | Supply LiveKit credentials, confirm Gemini Live access, configure `SYNTHIA_REALTIME_VOICE_ENABLED=true` and `SYNTHIA_REALTIME_VOICE_WORKER_READY=true`, then deploy the separate `voice-worker` on always-on/Reserved hosting before a user starts a session. This verification found the Gemini server credential configured, but both realtime flags and all three LiveKit settings are currently absent or disabled. | Browser microphone prompts, room connection, and live model use must remain explicit, user-started, and operator-configured; no real-time media session or provider consumption is permitted during development. |
...
## Voice Mode review, 2026-08-21

The implementation is present and protected by deterministic coverage: the focused Voice Mode configuration suite and workspace UI suite passed **40 assertions** without requesting microphone access, display-capture access, a LiveKit room, a Gemini session, or any external workload. The feature is therefore **implemented but not live-ready in this deployment**. It remains visibly unavailable until the required server-side configuration and separate always-on worker are deliberately completed.
