...
| Task orchestration | Implemented control plane | Typed task lifecycle, event model, policy gates, worker boundaries, automatic text/vision planning selection, and explicit manual-model override. | Enable only the providers and execution resources you intend to use. Unavailable or malformed planning routes pause safely before agent actions rather than requeueing side effects. |
