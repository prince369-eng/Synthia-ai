...
function configuredModelCatalogIncludes(provider: "agnes" | "aihubmix") {
  return ENV.availableModels.some(model => model.startsWith(`${provider}:`) || (!model.includes(":") && ENV.orchestratorProvider === provider));
}
...
export function systemServiceReadiness(): ServiceReadiness[] {
  return [
    ...
    service("agnes", "Agnes AI", "model", Boolean(ENV.agnesApiKey), ENV.orchestratorProvider === "agnes" || ENV.subtaskProvider === "agnes" || configuredModelCatalogIncludes("agnes"), ["AGNES_API_KEY", "AGNES_BASE_URL"]),
    service("aihubmix", "AIHubMix", "model", Boolean(ENV.aihubmixApiKey), ENV.orchestratorProvider === "aihubmix" || ENV.subtaskProvider === "aihubmix" || configuredModelCatalogIncludes("aihubmix"), ["AIHUBMIX_API_KEY", "AIHUBMIX_BASE_URL", "AIHUBMIX_FALLBACK_BASE_URL"]),
