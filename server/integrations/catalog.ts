...
export function systemServiceReadiness(): ServiceReadiness[] {
  return [
    ...
    service("hyperbrowser", "Hyperbrowser Agent Browser", "sandbox", Boolean(ENV.hyperbrowserApiKey), ENV.agentBrowserProvider === "hyperbrowser", ["HYPERBROWSER_API_KEY", "SYNTHIA_AGENT_BROWSER_PROVIDER", "SYNTHIA_HYPERBROWSER_TIMEOUT_MINUTES", "SYNTHIA_HYPERBROWSER_ALLOWED_HOSTS"]),
    service("pixazo", "Pixazo", "model", Boolean(ENV.pixazoApiKey && (ENV.pixazoImageModels.length > 0 || ENV.pixazoVideoModels.length > 0 || ENV.pixazoAudioModels.length > 0)), false, ["PIXAZO_API_KEY", "PIXAZO_IMAGE_MODELS", "PIXAZO_VIDEO_MODELS", "PIXAZO_AUDIO_MODELS", "SYNTHIA_PIXAZO_GENERATION_ENABLED"]),
