...
const skillVisibilitySchema = z.enum(["private", "workspace", "public_platform"]);
  ...
  skills: router({
        ...
        try {
          const completion = await generateWithFallback({
            ...
            messages: [
              {
                role: "system",
                content: "You draft a safe, reusable Synthia Skill for a user to review. Return only JSON with name, description, category, and skillMdContent. The Markdown must start with a heading and contain concise sections for Purpose, When to use, Instructions, and Safety boundaries. Do not include secrets, API keys, connector setup, claims of completed work, or instructions to bypass approval gates. A draft is never activated automatically.",
              },
              { role: "user", content: JSON.stringify({ idea: input.idea, category: input.category ?? "other" }) },
            ],
            maxTokens: 1_800,
          });
          ...
          }).parse(parseStructuredModelOutput(completion.content));
          return { ...parsed, isAutoGenerated: true };
        } catch (error) {
    ...
    create: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        try {
          const skillId = await createSkillForUser({ ...input, userId: ctx.user.id, visibility: "private" });
